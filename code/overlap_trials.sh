...
sed -ie "s/ TIME_STEADY .*/ TIME_STEADY (double) 5E3 /" "$temp_globals" ;
sed -ie "s/ TIME_WINDOW .*/ TIME_WINDOW (double) 0.50E3 /" "$temp_globals" ;
...
read n_pop N K dir n_trials kappa <<<  "$1 $2 $3 $4 $5 $6"

sed -i "s/IF_CHRISTOS .*/IF_CHRISTOS = 0 /" params.cfg ;
sed -i "s/IF_DPA .*/IF_DPA = 0 /" params.cfg ;
sed -i "s/IF_DUAL .*/IF_DUAL = 0 /" params.cfg ;
...
kappa=$(echo "print(${kappa} * 1.0)" | python3)
