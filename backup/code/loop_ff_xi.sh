...
for trial in $(seq 0 1 10); do 
    ...
    for var in $(seq 0.0 .05 .5); do 

	echo "#########################################################################"
	echo "trial ${trial}, var_ff ${var}"
	echo "#########################################################################"

	sed -ie "s/VAR_FF .*/VAR_FF ${var}/" globals.h 
	...
	echo "compiling ..."
	g++ main.cpp -std=c++11 -Ofast -s -o loop_ff_xi.out	

	screen -dmS  J0_${J0}_MEAN_XI_${J1}_VAR_XI_${sig1}_trial_${trial}_var_ff_${var} ./loop_ff_xi.out $dir $n_pop $N $K
	echo "running J0_${J0}_MEAN_XI_${J1}_VAR_XI_${sig1}_trial_${trial}_var_ff_${var}"
    done
