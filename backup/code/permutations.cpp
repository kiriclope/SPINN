...
long double **count_ ; 
unsigned long FIRST_ITEM = 0 ; //firts item in the permu 
unsigned long n_, i, j ; 
...
void get_perm_at_d(unsigned long n_, double dist, unsigned long *&out) { 
  ...
  unsigned long *sigma = new unsigned long[n_] ; 
...
int main(int argc, const char * argv[]) {
  ...
  n_ = (unsigned long) 28000 ; // Permutation length 
  double dist = .5 ;
  ...
  // for(unsigned long i=0;i<n_+1;i++) delete[] count_[i] ; 
  // free(count_) ; 
  // delete[] count_ ; 
  // delete[] sigma ; 
  return 0;
